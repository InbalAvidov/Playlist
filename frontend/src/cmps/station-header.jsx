...
export function StationHeader({ station, onSelectImg, saveChanges, deleteStation, isLikedSongsPage }) {
    ...
    async function onDeleteStation(ev) {
        ev.stopPropagation()
        setIsMenuOpen(false)
        try {
        ...
        <section className="station-header" onClick={onOpenEditor}>
            ...
            <div className="station-options" style={{ backgroundColor: `${isLikedSongsPage ? "rgb(80, 56, 160)" : color}` }} >
                ...
                {station?._id === currStation?._id && isPlaying ?
                    <button className="green-play-pause-btn pause" onClick={(ev) => onPauseStation(ev)}>
                        <svg role="img" height="24" width="24" aria-hidden="true" viewBox="0 0 24 24" data-encore-id="icon"><path d="M5.7 3a.7.7 0 00-.7.7v16.6a.7.7 0 00.7.7h2.6a.7.7 0 00.7-.7V3.7a.7.7 0 00-.7-.7H5.7zm10 0a.7.7 0 00-.7.7v16.6a.7.7 0 00.7.7h2.6a.7.7 0 00.7-.7V3.7a.7.7 0 00-.7-.7h-2.6z"></path></svg>
                    </button>
                    :
                    <button className="green-play-pause-btn" onClick={(ev) => onPlayStation(ev, station)}>
                ...
                {!isLikedSongsPage &&
                    <div className="station-menu-btn">
                        <svg fill="white" opacity={"0.5"} onClick={onToggleMenu} role="img" height="32" width="32" aria-hidden="true" viewBox="0 0 24 24" data-encore-id="icon"><path d="M4.5 13.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3zm15 0a1.5 1.5 0 100-3 1.5 1.5 0 000 3zm-7.5 0a1.5 1.5 0 100-3 1.5 1.5 0 000 3z"></path></svg>
            ...
            {isShareModal && <ShareModal setIsShareModal={setIsShareModal} users={users} user={user} onCloseShare={onCloseShare} />}
            {isEdit && <EditModal closeEditor={closeEditor} imgUrl={imgUrl} onUploadImg={onUploadImg} station={station} />}
            {isEdit && <main onClick={closeEditor} className="app-modal-editor"></main>}
            {isShareModal && <main onClick={closeEditor} className="app-modal-editor"></main>}
        </section>
    )
}